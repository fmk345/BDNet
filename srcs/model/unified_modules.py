...
class UNet(nn.Module):
    ...
    def __init__(self, in_channel, out_channel, ncolor=3, instance_norm=False):
        super(UNet, self).__init__()

        self.ncolor = ncolor
        out_channel = out_channel * ncolor
        # Encode
        self.conv_encode1 = self.contracting_block(
            in_channels=in_channel, out_channels=64, instance_norm=instance_norm)
        self.conv_maxpool1 = nn.MaxPool2d(kernel_size=2)
        self.conv_encode2 = self.contracting_block(
            in_channels=64, out_channels=128, instance_norm=instance_norm)
        self.conv_maxpool2 = nn.MaxPool2d(kernel_size=2)
        self.conv_encode3 = self.contracting_block(
            in_channels=128, out_channels=256, instance_norm=instance_norm)
        self.conv_maxpool3 = nn.MaxPool2d(kernel_size=2)
        # Bottleneck
        self.bottleneck = self.expansive_block(
            in_channels=256, mid_channels=512, out_channels=256, instance_norm=instance_norm)
        # Decode
        self.conv_decode3 = self.expansive_block(
            in_channels=512, mid_channels=256, out_channels=128, instance_norm=instance_norm)
        self.conv_decode2 = self.expansive_block(
            in_channels=256, mid_channels=128, out_channels=64, instance_norm=instance_norm)
        self.final_layer = self.final_block(
            in_channels=128, mid_channels=64, out_channels=out_channel, instance_norm=instance_norm)
    ...
    def forward(self, x):
        ...
        final_out = rearrange(final_out, 'b (n c) h w -> b n c h w', c=self.ncolor)
